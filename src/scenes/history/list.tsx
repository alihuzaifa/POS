...
import { Close, Delete, LocationOn, RemoveRedEye, WhatsApp } from "@mui/icons-material";
...
const HistoryList = () => {
    ...
    const deleteSellItem = () => {
        ...
        setList(filterUser)
        localStorage.setItem('billing', JSON.stringify(allItems))
        setDeleteObj({})
        setdeleteOpen(false)
    }
    const columns = [
    ...
    const init = () => {
        ...
        if (existingData) {
            const parsedData = JSON.parse(existingData)?.map((_obj: any, index: number) => {
                return { ..._obj, id: index + 1, invoiceNo: _obj?.invoiceNo }
            })
    ...
    return (
        <Box m="1.5rem 2.5rem">
            <FlexBetween>
                <Header title="HISTORY" subtitle="HISTORY OF ALL BILLS" />
            </FlexBetween>
            <Box
                mt="20px"
                display="grid"
            ...
            <CustomModal open={billView} size="lg" isHeight={true}>
                <Box sx={{
                    display: "flex",
                    justifyContent: 'end',
                    alignItems: "center"
                ...
                <Box sx={{
                    ...
                    <FlexBetween >
                        ...
                        >
                            TOTAL:{" "} <span style={{ marginLeft: '34px' }}>{bills.reduce((acc: number, { totalPrice }: any) => acc + totalPrice, 0) ?? 0}</span>
                        </Typography>
                    </FlexBetween>
                    <Typography
                        color={"black"}
