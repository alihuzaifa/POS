...
const KhataBackup = () => {
    ...
    const saveBackup = async () => {
        ...
        const promises = addMonthWithYearName.map(async (element) => {
            const q = query(
                collection(db, "khata-detail"),
                where("id", "==", Number(element?.id)),
            );
            const querySnapshot = await getDocs(q);
            const getResultArray: any[] = [];
            querySnapshot.forEach((doc) => {
                getResultArray.push(doc?.data());
            });
            const matchingObj = getResultArray.find(obj2 => obj2.id === element.id);
            if (matchingObj) {
                element.items.push(...matchingObj.items);
                const collectionRef = collection(db, "khata-detail");
        ...
        setList([]);
        toast.success('Khata Detail Has Been Saved Successfully');
        setOpen(false);
        setLoader(false);
    }
    ...
    const formik = useFormik<MyFormValues>({
        ...
        onSubmit: async (_values: any) => {
            ...
            const unsubscribe = onSnapshot(q, (querySnapshot) => {
                ...
                localStorage.setItem('khata-backup', JSON.stringify(data))
            });
            formik.resetForm();
            toast.success('Khata Get Successfully');
            return unsubscribe
        }
    });
    const months = [
        ...
        render={({ online }) => (
            <div>
                {online ? (
                    <Box m="1.5rem 2.5rem">
                        <FlexBetween>
                            <Header title="KHATA BACKUP" subtitle="Save Khata Detail For Future Use" />
                            {
                                list?.length > 0 &&
                                <Button
                                    sx={{
                        ...
                        <CustomModal open={open} size="sm">
                            <Typography variant="h5" color={theme.palette.secondary[300]} textAlign={'center'}>
                                Are you sure you want to save khata detail?
                            </Typography>
